...
# which is a library that can be dynamically loaded at runtime
# by programs that depend on it.
...
# -L . : is an option that tells the linker to search for shared libraries
# in the current directory (represented by .)
# This is necessary when linking with a shared library that is
# not installed in a system directory.
# -l dynamic :tells the linker to link with the shared library
# named "lib + name_you_provide +.so",
# which provides implementation for some of the functions called in 0-main.c.
# Notice that this option should be after 0-main.c

# tip: ldd a.out =>  print shared object dependencies
# prints  the shared objects (shared libraries) required by each program
# or shared object specified on the command line.

# 3- to load your shared lib during runtime
# * you have to add the location of your library files
#  into the environmental variable LD_LIBRARY_PATH
# to know where to find the functions.
